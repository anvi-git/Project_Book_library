...
# ── 2. Design tokens (Our World in Data–inspired) ───────────
ACCENT        = "#01696f"   # OWID teal primary
ACCENT_LIGHT  = "#4f98a3"
ACCENT_PALE   = "#cedcd8"
...
# OWID-style categorical palette (earth tones + teal anchors)
OWID_PALETTE = [
...
# Shared Plotly layout — mimics OWID's clean editorial charts
CHART_LAYOUT = dict(
    font=dict(family="Playfair Display, Georgia, serif", color=TEXT_MAIN, size=13),
    ...
    xaxis=dict(
        showgrid=True, gridcolor="#ede9e3", gridwidth=1,
...
app = Dash(
    ...
    external_stylesheets=[FONT_IMPORT],
...
GLOBAL_CSS = f"""
  * {{ box-sizing: border-box; margin: 0; padding: 0; }}
  ...
  .kpi-card:hover {{
    box-shadow: 0 6px 20px rgba(1,105,111,.13);
  ...
  .chart-card:hover {{
    box-shadow: 0 4px 16px rgba(1,105,111,.10);
  }}
  .filter-label {{
    font-family: 'Lato', sans-serif;
    font-size: 11px;
...
def update_charts(langs, sexes, year_range):
    ...
    fig_scatter = px.scatter(
        ...
        color_discrete_sequence=OWID_PALETTE,
        opacity=0.75,
    )
    ...
    fig_pie.update_layout(**CHART_LAYOUT,
                           title_text="Top 10 author nationalities",
                           showlegend=False)
    ...
    fig_tree = px.treemap(
        pub_counts,
        path=["publisher_clean"], values="books",
